...
rule combine_test_celltype_frequencies:
    input:
        tsvs = expand('data/{modality}/imbalance/test-breakdown/{bal}-{similarity}-imbalance-test-seed-{s}.tsv',
            modality = modalities, bal = dataset_imbalance, similarity = similarity_vals, s = train_test_seeds)
    priority: 2
    output:
...
rule benchmark_imbalance:   
    ...
    shell:
        ...
        "params = list(sn_acc = '{input.sn_acc}', sc_acc = '{input.sc_acc}', cy_acc = '{input.cy_acc}'))\" "
