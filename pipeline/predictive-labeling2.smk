...
def get_mem_mb_pred_labs(wildcards, attempt):
    return attempt * 2000 + 8000

def get_mem_mb_scmap(wildcards, attempt):
    return attempt * 2000 + 1000
...
def get_pred_labeling(modality, cell_selection = ['top10', 'top50', 'top100'], selection_expansion_dict = selection_expansion_dict, evaluation_methods_dict = evaluation_methods_dict, cell_numbers = cell_numbers):    
    if modality == 'scRNASeq' or modality == 'snRNASeq':
        data = []
...
pred_lab2 = {
    'pred_labs': expand(get_pred_labeling("CyTOF") + get_pred_labeling("scRNASeq") + get_pred_labeling("snRNASeq"), s = train_test_seeds),
    'acc': expand(output + 'new/benchmark-predictive-labeling-{modality}-seed-{s}.tsv', modality = ['scRNASeq', 'CyTOF', 'snRNASeq'], s = train_test_seeds),
    'benchmark': expand(output + 'reports/pred2/benchmark-predictive-labeling-{modality}.html', modality = ['scRNASeq', 'CyTOF', 'snRNASeq']),
    'benchmark2': expand(output + 'new/pred2/benchmark-predictive-labeling-{modality}.tsv', modality = ['scRNASeq', 'CyTOF', 'snRNASeq']),
