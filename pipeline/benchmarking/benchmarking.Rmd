...
acc %>% 
  ...
  ggplot(aes(x = cell_num, y = mean_estimate, color = selection_procedure)) +
  geom_point() +
  ylim(0, 1) +
  geom_line() +
...
lapply(unique(acc$method), function(x){
  acc %>% 
    filter(rand == "0") %>% 
    filter(method == x) %>% 
    ggplot(aes(x = as.character(corrupted), y = .estimate, color = selection_procedure, group = selection_procedure)) + 
    geom_point() +
    geom_line() +
    ylim(0, 1) +
    facet_grid(cell_num ~ .metric) + 
...
lapply(unique(acc$method), function(x){
  acc %>% 
    ...
    facet_grid(cell_num ~ .metric) + 
    theme_bw() + 
    labs(title = x, color = "Selection procedure", x = "Random") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
})
```
